...
La question dans ce dossier est : quels sont les facteurs importants impactant la gravité des accidents en France en 2021 ?
...
colors_gravite <- c("Indemne" = "green",
                    "Tué" = "red",
                    "Blessé hospitalisé" = "orange",
                    "Blessé léger" = "purple")

df_surf$grav <- factor(df_surf$grav,levels = 1:4,
                       labels =  c("Indemne", "Tué", "Blessé hospitalisé", "Blessé léger"))

df_surf$surf <- factor(df_surf$surf, levels = 1:8,
                       labels = c("Normale", "Mouillée","Flaques","Inondée","Enneigée","Boue","Verglacée","Corps gras"))
            
ggplot(df_surf, aes(x = surf)) +
  geom_col(position = "dodge", aes(y = n_accidents, fill = as.factor(grav))) +
  scale_fill_manual(values = colors_gravite, name = "Surface",
                    labels = c("Indemne", "Tué", "Blessé hospitalisé", "Blessé léger")) +
  labs(title = "Nombre d'accidents par rapport au type de surface de la route", x = "", y = "Nombre d'accidents")
