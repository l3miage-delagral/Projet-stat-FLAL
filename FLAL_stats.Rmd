...
Sur 126086 accidents de la route en 2021, 50 % des conducteurs ont moins de 50.5 ans. 50 % d'entre eux ont entre 25.5 et 77.5 ans.


Dans ce cas, l'intervalle de confiance est [42.6, 58.4], ce qui signifie que l'on peut être raisonnablement sûr que la moyenne de l'âge dans la population dont l'échantillon a été prélevé se trouve dans cette plage avec une probabilité de 95%.
...
ggplot(data = plot_usagers, aes(x=age, y=count, fill=grav)) +
  geom_bar(stat="identity") + geom_smooth(se=FALSE, color = "orange") + ggtitle("Effectif des accidents mortels par âge")+    theme(legend.position='none') + xlab("Âge") + ylab("Effectif")
...
colors <- c("Indemne" = "#00BFFF", 
            "Blessé léger" = "#FFD700", 
            "Blessé hospitalisé" = "#FF6347", 
            "Tué" = "#800000")

# Créer le graphique
ggplot(df_long, aes(x = sexe, fill = grav)) +
  geom_col(position = "dodge", aes(y = count)) +
  scale_fill_manual(values = colors, name = "Gravité de l'accident",
                    labels = c("Indemne", "Blessé léger", "Blessé hospitalisé", "Tué")) +
  labs(title = "Gravité des accidents par sexe", x = "Sexe", y = "Nombre d'accidents")
...
chisq.test(table(sub_df$sexe, sub_df$grav))
