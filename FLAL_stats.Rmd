...
Dans ce rapport, nous cherchons à expliquer la gravité des accidents en France en 2021. Le jeu de données vient du site data.gouv.fr.
La question dans ce dossier est : 

### <strong>Quels sont les facteurs importants impactant la gravité des accidents en France en 2021 ?</strong>


Pour trier le jeu de données, nous avons d'abord commencé par enlever les lignes de la dataframe sans données dans lesquelles les colonnes étaient remplies de -1. On a ensuite trié les colonnes pour supprimer les colonnes que nous n'allons pas regarder dans notre études comme par exemple le type de moteur du véhicule. Pour finir il fallait enlever les caractères spéciaux des régions. Après tout cela nous avons un jeu de données propre et exploitable.

En ce qui concerne la représentation des données on a décidé de représenter toutes les données dans une grande dataframe dont on récupérera les colonnes necessaires pour les différents facteurs.
...
Ici on voit un gros pique en juin qui peut s'éxpliquer par la fin du confinement de 2021 ce qui a poussé les gens à sortir et donc à plus prendre la voiture donc forcément il y plus d'accidents.
...
```{r echo=FALSE, warning=FALSE}
# Sous-ensemble de données contenant l'âge et la gravité d'accidents pour chaque âge
sub_df <- df %>% filter(place == 1, catu == 1, age > 13, grav == 1) %>%
  group_by(age) %>% 
  summarise(grav = n()) %>% 
  filter(!is.na(age))
# Créer un boxplot avec le nombre d'accidents sur l'axe x et l'âge sur l'axe y
ggplot(sub_df, aes(x = grav, y = age)) +
  geom_boxplot() +
  ylab("Âge du conducteur") +
  xlab("Nombre d'accidents") +
  coord_flip() + ggtitle("Boxplot sur l'âge des conducteurs/conductrices indemnes")
# Afficher les statistiques du boxplot
print(boxplot.stats(sub_df$age))
```
```{r echo=FALSE, warning=FALSE}
# Sous-ensemble de données contenant l'âge et la gravité d'accidents pour chaque âge
sub_df <- df %>% filter(place == 1, catu == 1, age > 13, grav == 2) %>%
  group_by(age) %>% 
  summarise(grav = n()) %>% 
  filter(!is.na(age))
# Créer un boxplot avec le nombre d'accidents sur l'axe x et l'âge sur l'axe y
ggplot(sub_df, aes(x = grav, y = age)) +
  ...
  ylab("Âge du conducteur") +
  xlab("Nombre d'accidents") +
  coord_flip() + ggtitle("Boxplot sur l'âge des conducteurs/conductrices tués")
# Afficher les statistiques du boxplot
print(boxplot.stats(sub_df$age))
```
```{r echo=FALSE, warning=FALSE}
# Sous-ensemble de données contenant l'âge et la gravité d'accidents pour chaque âge
sub_df <- df %>% filter(place == 1, catu == 1, age > 13, grav == 3) %>%
  group_by(age) %>% 
  summarise(grav = n()) %>% 
  filter(!is.na(age))
# Créer un boxplot avec le nombre d'accidents sur l'axe x et l'âge sur l'axe y
ggplot(sub_df, aes(x = grav, y = age)) +
  ...
  ylab("Âge du conducteur") +
  xlab("Nombre d'accidents") +
  coord_flip() + ggtitle("Boxplot sur l'âge des conducteurs/conductrices blessés hospitalisés")
# Afficher les statistiques du boxplot
print(boxplot.stats(sub_df$age))
```
```{r echo=FALSE, warning=FALSE}
# Sous-ensemble de données contenant l'âge et la gravité d'accidents pour chaque âge
sub_df <- df %>% filter(place == 1, catu == 1, age > 13, grav == 4) %>%
  group_by(age) %>% 
  summarise(grav = n()) %>% 
  filter(!is.na(age))
# Créer un boxplot avec le nombre d'accidents sur l'axe x et l'âge sur l'axe y
ggplot(sub_df, aes(x = grav, y = age)) +
  ...
  ylab("Âge du conducteur") +
  xlab("Nombre d'accidents") +
  coord_flip() + ggtitle("Boxplot sur l'âge des conducteurs/conductrices blessés légers")
# Afficher les statistiques du boxplot
print(boxplot.stats(sub_df$age))
```
...
table(df_secu$grav, df_secu$secu1)
```
